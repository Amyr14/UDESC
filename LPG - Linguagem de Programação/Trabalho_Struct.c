...
int choque_evento( struct Evento agenda[], struct Evento *evento, int num_eventos );

int main() {
    int opcao, rodando = 1, num_eventos = 0;
        ...
        switch ( opcao ) {
            ...
            case 6:
                ...
                for (int i = 0; i < num_eventos; i++)
                    fprintf(arq_saida,"%d\n%d\n%d\n%d\n%d\n%d\n%d\n%s%s", agenda[i].data.dia, agenda[i].data.mes, agenda[i].data.ano,
                            agenda[i].inicio.hora, agenda[i].inicio.min, agenda[i].fim.hora, agenda[i].fim.min, agenda[i].local, agenda[i].descricao);
                fclose(arq_saida);
                rodando = 0;
                break;
            default:
...
void cadastrar_evento( struct Evento agenda[], int num_eventos ) {
    ...
    pega_evento( &evento );
    while( choque_evento( agenda, &evento, num_eventos ) ) {
    ...
    agenda[ num_eventos ] = evento;
    ord_evento( agenda, num_eventos + 1 );
}
...
void printa_evento( struct Evento evento ) {
    ...
    printf("Horario de ecerramento: %d:%d\n", evento.fim.hora,
                                              evento.fim.min );
    printf("Local: ");
    puts( evento.local );
    printf("------------------\n");
...
void remover_evento( struct Evento agenda[], int *num_eventos ) {
    ...
    for ( i = 0; i < *num_eventos; i++ )
        if ( comp_data( &agenda[i].data, &data ) == 0 && comp_horario( &agenda[i].inicio, &horario ) == 0 ) {
...
int choque_evento( struct Evento agenda[], struct Evento *evento, int num_eventos ) {
    ...
    for ( i = 0; i < num_eventos; i++ )
        if ( comp_data( &evento->data, &agenda[i].data ) == 0 
          && comp_horario( &evento->inicio, &agenda[i].inicio ) == 0 )
          return 1;
    return 0;
}
...
int data_valida( struct Data *data ) {
    return data->dia >= 1 && data->dia <= 31 && data->mes >= 1 && data->mes <= 12 && data-> ano >= 2022;
}

int horario_valido( struct Horario *horario ) {
    return horario->hora >= 0 && horario->hora <= 23 && horario->min >= 0 && horario->min <= 59;
}

void ord_evento( struct Evento agenda[], int num_eventos ) {
    int i, j;
    struct Evento aux;
    for ( i = 0; i < num_eventos - 1; i++ ) 
        for ( j = i + 1; j < num_eventos; j++ )
            if ( comp_data( &agenda[j].data, &agenda[i].data ) == 1 
                || (comp_data( &agenda[j].data, &agenda[i].data ) == 0 
                && comp_horario( &agenda[j].inicio, &agenda[i].inicio ) == 1 ) ) {
                    aux = agenda[i];
                    agenda[i] = agenda[j];
                    agenda[j] = aux;
                }
